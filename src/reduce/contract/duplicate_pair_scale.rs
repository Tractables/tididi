...
fn push_count_slot(eng: &Engine, tdd: &mut Tdd, mv: VtreeIdx, val: Count) -> Result<u32, ApplyError> {
    ...
    // reads a bare marginal-side ref at a LEAF as a leaf-LABEL (fixed count), never
...
fn try_scale_child(
    ...
    );
    {
        ...
        // there a weighted analogue of `scale_leaf_marginal_label`'s inline absorb: a
        ...
        // canonicalization the leaf-marginal pass now runs — is the column itself:
