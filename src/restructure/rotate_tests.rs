...
fn parent_of_marginal_rotation_preserves_model_count() {
    ...
    marginalize_batch(&lim, &mut tdd, &targets, &vtree).expect("no wall is installed in a test");
    ...
    marginalize_closure(&lim, &mut tdd, &new_vtree).expect("no wall is installed in a test");
    minimize(&mut tdd);
...
fn fuzz_search_preserves_marginal_count() {
    ...
    for seed in 0..600u64 {
        ...
        marginalize_batch(&lim, &mut tdd, &[tgt], &vtree).expect("no wall is installed in a test");
        let mc_before = model_count(&tdd);

        crate::restructure::search::search_to_local_min(&mut tdd);
        let vt = tdd.vtree.clone();
        marginalize_closure(&lim, &mut tdd, &vt).expect("no wall is installed in a test");
        let mc_after = model_count(&tdd);
...
fn gc1_sweep_undercount_repro() {
    ...
    marginalize_batch(&lim, &mut tdd, &[VtreeIdx(9)], &vtree).expect("no wall is installed in a test");
    ...
    let vt = tdd.vtree.clone();
    let closed = marginalize_closure(&lim, &mut tdd, &vt).expect("no wall is installed in a test");
...
fn rotate_left_and_check_locality(lim: &Limits, tdd: &mut Tdd, target: crate::vtree::VtreeIdx) -> Option<(usize, usize)> {
...
fn rotate_right_and_check_locality(lim: &Limits, tdd: &mut Tdd, target: crate::vtree::VtreeIdx) -> Option<(usize, usize)> {
    use crate::reduce::minimize_after_rotation;
    let mut vt = (*tdd.vtree).clone();
...
fn rotation_locality_left_at_root_balanced5() {
    ...
    let _ = rotate_left_and_check_locality(&lim, &mut tdd, root)
...
fn rotation_locality_right_after_left_balanced5() {
    ...
    let root = tdd.vtree.root();
    let _ = rotate_right_and_check_locality(&lim, &mut tdd, root)
...
fn rotation_locality_linear_left_chain() {
    ...
    for _ in 0..3 {
        let root = tdd.vtree.root();
        if rotate_left_and_check_locality(&lim, &mut tdd, root).is_none() {
            break;
        }
    }
}
...
fn rotation_locality_unsat_left() {
    ...
    let root = tdd.vtree.root();
    if rotate_left_and_check_locality(&lim, &mut tdd, root).is_some() {
        // Asserted internally.
    }
}
