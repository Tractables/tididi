...
pub fn is_sat(tdd: &Tdd) -> bool {
    // ZERO sentinel means the TDD computes the constant-false function.
    if tdd.is_zero() {
        return false;
    }
    let out_vtree = tdd.output.vtree;
    if tdd.vtree.node(out_vtree).is_leaf() {
        // Implicit leaf: any index in {One=0, Pos=1, Neg=2} is satisfiable.
        true
    } else {
        let out_level = &tdd.levels[out_vtree.idx()];
        let out_node = &out_level.nodes[tdd.output.local.idx()];
...
///
/// Unlike [`is_sat`], which is O(1) but *assumes a reduced/minimized diagram* (output
...
/// model count — restoring the [`Tdd::is_zero`]/[`is_sat`] invariant that downstream
/// applies rely on.
pub(crate) fn output_is_satisfiable(tdd: &Tdd) -> bool {
    if tdd.is_zero() {
        return false;
    }
    let vtree = &tdd.vtree;
