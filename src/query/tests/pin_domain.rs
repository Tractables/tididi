    ...
    for tree in [Vtree::balanced_over(&vars), Vtree::linear_from_order(&vars), rotated] {
        ...
        for (f, kind) in [(Tdd::one(&tree), 0), (Tdd::zero(&tree), 1), (Tdd::clause(&tree, [3]).unwrap(), 2)] {
            ...
            for semantics in [PinSemantics::Evidence, PinSemantics::Cofactor] {
                let mut counter = ModelCounter::<R>::new_on(&eng, &f, semantics).unwrap();
                for code in (0..27).chain(std::iter::once(0)) {
                    ...
                    assert_eq!(counter.model_count_on(&eng).unwrap(), BigUint::from(expected));
                    assert_eq!(counter.model_count_on(&eng).unwrap(), BigUint::from(expected));
                }
            }
        }
...
fn invalid_pins<R: Retention>() {
    ...
    for pending in [false, true] {
        ...
        assert_eq!(counter.model_count_on(&eng).unwrap(), if pending { 2u32.into() } else { 4u32.into() });
    }
...
fn sparse_pin_storage_fits_a_budget_independent_of_variable_ids() {
    ...
    assert_eq!(counter.model_count_on(&eng).unwrap(), 2u32.into());
}
