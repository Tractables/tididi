...
fn take<'a>(eng: &'a Engine, pool: &'a Pool<MarkBuffer>, num_nodes: usize) -> Result<PoolGuard<'a, MarkBuffer>, OperationError> {
    let mut marks = pool.checkout(eng);
    marks.cover(eng.limits(), num_nodes)?;
    Ok(marks)
}
...
fn interrupted_ancestor_walk_returns_clean_flags_for_retry() {
    ...
    {
        let mut flags = take(&eng, &pool, vtree.num_nodes()).unwrap();
...
fn mark_reports_first_visit_and_nested_checkouts_keep_independent_marks() {
    let eng = Engine::new();
    let pool = Pool::default();
    let mut outer = take(&eng, &pool, 8).unwrap();
    assert!(outer.mark(VtreeIdx(3)));
    assert!(!outer.mark(VtreeIdx(3)));
    {
        let mut inner = take(&eng, &pool, 8).unwrap();
    ...
    assert!(!outer[5]);
    drop(outer);
    let reused = take(&eng, &pool, 8).unwrap();
