...
/// Flat row offset `a * k2` of child node row `a` in a `k2`-column child grid.
///
...
#[inline(always)]
fn child_grid_mul(a: u32, k2: u32) -> usize {
    a as usize * k2 as usize
}
...
/// Existing grid representation: flat `node_idx[base + row*k2 + col]`.
...
impl ChildLookup for DenseLookup {
    ...
    fn get(&self, node_idx: &[u32], row: u32, col: u32) -> u32 {
        ...
        // [base, base + k1*k2) slab — identical access to the historical
        // hand-written `get_unchecked`. `child_grid_mul` widens before the
        // multiply so the index is exact on 64-bit targets.
        unsafe {
            *node_idx.get_unchecked(self.base + child_grid_mul(row, self.k2) + col as usize)
        }
    }
}
...
/// pass-through side the raw operand field (`row` = the c1 pair field, `col` =
/// the c2 pair field) is carried verbatim — it is an inline model count or a
...
pub(super) struct MargLookup {
    ...
    k2: u32,
    passthrough: bool,
    /// Carrier selector when `passthrough`: true ⇒ carry the c1 field (`row`).
    pt_c1: bool,
}
...
impl ChildLookup for MargLookup {
    ...
    fn get(&self, node_idx: &[u32], row: u32, col: u32) -> u32 {
        if self.passthrough {
            ...
            // [base, base + k1*k2) slab. `child_grid_mul` widens before the
            // multiply so the index is exact on 64-bit targets.
            unsafe {
                *node_idx.get_unchecked(self.base + child_grid_mul(row, self.k2) + col as usize)
            }
