...
fn conjunction_returns_workspace_after_refusal_and_reuses_it() {
    ...
    assert_canonical(&f);
    let allocation = eng.scratch.apply.workspace.checkout(&eng).f_widths.as_ptr();
    let mut refusals = 0;
    for cut in 0..20 {
        ...
        assert_eq!(eng.scratch.apply.workspace.checkout(&eng).f_widths.as_ptr(), allocation);
...
fn ordinary_conjunctions_reuse_the_context_workspace() {
    ...
    let allocation = vtree.context().run(|eng| {
        eng.scratch.apply.workspace.checkout(eng).f_widths.as_ptr()
    });
...
fn returning_the_workspace_trims_the_width_arrays() {
    ...
    let eng = Engine::new();
    {
        let mut ws = eng.scratch.apply.workspace.checkout(&eng);
    ...
    let ws = eng.scratch.apply.workspace.checkout(&eng);
