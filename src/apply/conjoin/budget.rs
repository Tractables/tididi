//! The apply engine's own accounting on top of [`crate::limits`]: the
...
#[inline(always)]
pub(super) fn try_push_pair_into(level: &mut crate::diagram::TddLevel, pair: crate::diagram::InputPair) -> Result<(), ApplyError> {
...
const PAIR_ELEM_BYTES: u64 = std::mem::size_of::<crate::diagram::InputPair>() as u64;
...
#[cold]
#[inline(never)]
fn grow_pairs_bounded(v: &mut Vec<crate::diagram::InputPair>) -> Result<(), ApplyError> {
    let cap = v.capacity();
    if cap == 0 {
