...
pub(super) struct ChildPlan<'a> {
    ...
    pub base: usize,
    /// c2 column count for this child's grid (its row stride).
    pub k2: u32,
    /// Per-c1-row live-column bitmasks (indexed by c1 child node idx).
    pub live_cols: &'a [u128],
    /// Per-j reach bitmasks for c2's references to this child.
    pub reach: &'a [u128],
}
