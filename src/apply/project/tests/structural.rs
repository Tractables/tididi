...
fn regrouping_checks_its_allocations_before_reduction() {
    ...
    let result = exists_leaves_structural(&eng, original, &[leaf], false).unwrap();
...
fn freeing_a_whole_subtree_matches_one_variable_at_a_time() {
    ...
    let eng = Engine::new();
    let at_once = exists_leaves_structural(&eng, f.clone(), &block, false).unwrap();
    assert_canonical(&at_once);
    let mut one_at_a_time = f;
    for &leaf in &block {
        one_at_a_time = exists_leaves_structural(&eng, one_at_a_time, &[leaf], false).unwrap();
    }
...
fn quantifying_every_variable_leaves_the_constant() {
    ...
    let eng = Engine::new();
    let result = exists_leaves_structural(&eng, f, &all, false).unwrap();
    assert_canonical(&result);
    assert_eq!(result.model_count().unwrap(), 16u32.into());
}
...
fn structural_projection_recovers_after_each_refused_reservation() {
    ...
    for nth in 0..512 {
        ...
        match exists_leaves_structural(&eng, f.clone(), &[leaf], false) {
            Ok(result) => { assert_canonical(&result); reached_success = true; break; }
            Err(error) => assert_eq!(error, OperationError::OverBudget),
        }
        eng.limits().grant_every_reserve();
        let result = exists_leaves_structural(&eng, f.clone(), &[leaf], false).unwrap();
        assert_canonical(&result);
...
fn quantifying_an_unreduced_product_matches_quantifying_a_reduced_one() {
    ...
    let eng = Engine::new();
    let from_product = exists_leaves_structural(&eng, product, &leaves, false).unwrap();
    let from_reduced = exists_leaves_structural(&eng, reduced, &leaves, false).unwrap();
