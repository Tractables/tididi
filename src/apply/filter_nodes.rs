...
/// A removed node in a level's remap row: rejected, a tombstone, or emptied.
...
/// at each rejected node and tombstone. `None` when every node was kept.
fn ask(eng: &Engine, f: &Tdd, mut keep: impl FnMut(TddNodeId) -> bool) -> Result<Option<Vec<Vec<u32>>>, OperationError> {
    ...
    for (t, _, _) in f.vtree.internal_bottomup() {
        ...
        for (i, node) in level.nodes.iter().enumerate() {
            poll.poll(1)?;
            let row = &mut remap[t.idx()];
            if !node.is_internal() {
                row[i] = DEAD;
            } else if !keep(TddNodeId { vtree: t, local: NodeIdx(i as u32) }) {
                row[i] = DEAD;
                rejected = true;
            }
        }
...
/// level's inlined-side markers come along.
    ...
    *out.parts_mut().1 = f.weights.as_ref().map(WeightStore::empty_like);
    ...
    for (t, left, right) in f.vtree.internal_bottomup() {
        ...
        for side in [ChildSide::Left, ChildSide::Right] {
            level.set_marginal_inlined(side, source.marginal_inlined(side));
        }
    }
    poll.flush()?;
    lim.discard(pairs);
