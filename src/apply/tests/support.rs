...
fn support_bits_covers_support_mask_and_detects_disjoint() {
    ...
    let g = clause_to_tdd(&vtree, &crate::test_helpers::clause(&[(1, true)]));
...
fn condition_var_on_marginalized_leaf_fails_fast() {
    ...
    let mut t = and2(
        &clause_to_tdd(&vtree, &crate::test_helpers::clause(&[(0, true), (1, true)])),
        &clause_to_tdd(&vtree, &crate::test_helpers::clause(&[(0, false), (1, false)])),
    );
...
fn implied_literals_matches_condition_oracle() {
    ...
    let or12 = clause_to_tdd(&vtree, &crate::test_helpers::clause(&[(1, true), (2, true)]));
