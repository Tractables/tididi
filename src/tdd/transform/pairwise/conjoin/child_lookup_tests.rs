...
/// row-offset multiply `a * k2` MUST widen to `usize` before multiplying.
...
fn child_grid_mul_does_not_wrap_past_u32() {
    let a: u32 = 70_000;
    let k2: u32 = 70_000;
    assert_eq!(child_grid_mul(a, k2), 4_900_000_000usize);
    // Sanity: the wrapped (buggy) value is what we must NOT produce.
    let wrapped = a.wrapping_mul(k2) as usize;
    assert_eq!(wrapped, 605_032_704usize);
    assert_ne!(child_grid_mul(a, k2), wrapped);
}
