...
use crate::tdd::types::{ExtMulti, InputPair, Tdd, TddNodeData, ZERO};
use crate::vtree::{VarId, VtreeIdx, VtreeNode};
use crate::tdd::transform::unary::project::{POS, NEG, ONE};
...
pub(crate) fn condition_leaf(t: &Tdd, leaf_idx: VtreeIdx, polarity: Polarity) -> Tdd {
...
/// `TddLevel::clear` — this leaves `marg_flags`, `retired_marg_width` and the
...
fn canonicalize_false_output(tdd: &mut crate::tdd::types::Tdd) {
    if tdd.is_zero() {
        return;
    }
    ...
    if !sat {
        tdd.output.local = crate::tdd::types::ZERO;
    }
}
