...
fn parent_of_marginal_rotation_preserves_model_count() {
    use crate::tdd::transform::unary::marginalize::{marginalize_batch, marginalize_closure};
    ...
    marginalize_batch(&mut tdd, &targets, &vtree).expect("no wall is installed in a test");
    ...
    marginalize_closure(&mut tdd, &new_vtree).expect("no wall is installed in a test");
    minimize(&mut tdd);
...
/// [`search_to_local_min`](crate::tdd::restructure::search::search_to_local_min)
...
fn fuzz_search_preserves_marginal_count() {
    ...
    for seed in 0..600u64 {
        ...
        marginalize_batch(&mut tdd, &[tgt], &vtree).expect("no wall is installed in a test");
        let mc_before = model_count(&tdd);

        crate::tdd::restructure::search::search_to_local_min(&mut tdd);
        let vt = tdd.vtree.clone();
        marginalize_closure(&mut tdd, &vt).expect("no wall is installed in a test");
        let mc_after = model_count(&tdd);
...
fn gc1_sweep_undercount_repro() {
    ...
    marginalize_batch(&mut tdd, &[VtreeIdx(9)], &vtree).expect("no wall is installed in a test");
    ...
    let vt = tdd.vtree.clone();
    let closed = marginalize_closure(&mut tdd, &vt).expect("no wall is installed in a test");
...
fn rotate_left_and_check_locality(tdd: &mut Tdd, target: crate::vtree::VtreeIdx) -> Option<(usize, usize)> {
...
fn rotate_right_and_check_locality(tdd: &mut Tdd, target: crate::vtree::VtreeIdx) -> Option<(usize, usize)> {
    use crate::tdd::minimize::minimize_after_rotation;
    let mut vt = (*tdd.vtree).clone();
...
fn rotation_locality_left_at_root_balanced5() {
    ...
    let _ = rotate_left_and_check_locality(&mut tdd, root)
...
fn rotation_locality_right_after_left_balanced5() {
    ...
    let root = tdd.vtree.root();
    let _ = rotate_right_and_check_locality(&mut tdd, root)
...
fn rotation_locality_linear_left_chain() {
    ...
    for _ in 0..3 {
        let root = tdd.vtree.root();
        if rotate_left_and_check_locality(&mut tdd, root).is_none() {
            break;
        }
    }
}
...
fn rotation_locality_unsat_left() {
    ...
    let root = tdd.vtree.root();
    if rotate_left_and_check_locality(&mut tdd, root).is_some() {
        // Asserted internally.
    }
}
