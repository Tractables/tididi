...
fn fill(
    ...
    for t in vtree.bottomup() {
        ...
        let finished = match (width, one_sided_child(vtree, layout, t)) {
            ...
            _ if width == num_vars => {
                close_subtree(eng, builder, vtree, &state, &mut scratch, t, sorted, w)?
            }
            _ => {
                ...
                store_level(eng, builder, vtree, &state, &mut scratch, t, atoms, sorted, w)?
            }
        };
        if !leaf {
...
fn close_subtree(
    eng: &Engine, builder: &mut TddBuilder, vtree: &Vtree,
    ...
    pairs.dedup();
    lim.check_stop()?;
    builder.reserve(eng, t, 1, pairs.len())?;
    let mut locals = Vec::new();
    lim.reserve_exact(&mut locals, 1)?;
    locals.push(builder.push(eng, t, pairs)?);
...
fn store_level(
    ...
    if atoms.count == scratch.run_atoms.len() {
        ...
        builder.reserve(eng, t, atoms.count, atoms.count)?;
        let mut gate = lim.gate();
        for &(row, atom) in &scratch.run_atoms {
            ...
            locals.push(builder.push(eng, t, &[ChildPair::new(l, r)])?);
        }
        gate.flush()?;
